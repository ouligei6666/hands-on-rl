...
# Gym registry API differs across versions (dict vs. Registry class)
registry = envs.registry
if hasattr(registry, "all"):
    env_specs = [env_spec for env_spec in registry.all()
                 if 'gym.envs' in env_spec.entry_point]
else:
    env_specs = [env_spec for env_spec in registry.values()
                 if 'gym.envs' in env_spec.entry_point]
for spec in env_specs:
    spec_kwargs = spec.kwargs if hasattr(spec, "kwargs") else spec._kwargs
    register(
        id='ma_' + spec.id,
        entry_point='ma_gym.envs.openai:MultiAgentWrapper',
        kwargs={'name': spec.id, **spec_kwargs}
    )
